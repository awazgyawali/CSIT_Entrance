...
public class VotesLeaderboard extends Fragment {
        ...
        FirebaseFirestore.getInstance().collection("users").document(FirebaseAuth.getInstance().getCurrentUser().getUid()).get().addOnCompleteListener(new OnCompleteListener<DocumentSnapshot>() {
            ...
            public void onComplete(@NonNull Task<DocumentSnapshot> task) {
                if (task.isSuccessful()) {
                    if (task.getResult().get("likesCount") != null)
                        myScore.setText(task.getResult().get("likesCount").toString());
                }
            }
        });
    ...
    private void fetchFromInternet() {
        ...
        FirebaseFirestore.getInstance().collection("users").orderBy("likesCount", Query.Direction.DESCENDING).limit(10)
                    ...
                    public void onEvent(@Nullable QuerySnapshot queryDocumentSnapshots, @javax.annotation.Nullable FirebaseFirestoreException e) {
                        try {
                            ...
                            for (QueryDocumentSnapshot document : queryDocumentSnapshots) {
                                ...
                                userDetail.put("score", String.valueOf(user.get("likesCount")));
                                userDetail.put("image_link", user.get("image_url"));
                                if (i < 3) {
                                    topNames[i].setText((String) user.get("name"));
                                    topScores[i].setText(String.valueOf(user.get("likesCount")));
                                    Picasso.with(getContext())
                                    ...
                                    scores.add((Long) user.get("likesCount"));
